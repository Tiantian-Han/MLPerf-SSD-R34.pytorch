...
class Encoder(object):
    ...
    def decode_batch_(self, bboxes_in, scores_in,  criteria = 0.45, max_output=200):
        #bboxes, probs = self.scale_back_batch(bboxes_in, scores_in)

        output = []
        for bbox, prob in zip(bboxes_in.split(1, 0), scores_in.split(1, 0)):
            bbox = bbox.squeeze(0)
            prob = prob.squeeze(0)
...
class DefaultBoxes(object):
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, mask, \
