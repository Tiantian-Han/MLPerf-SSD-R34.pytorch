...
class SSD(nn.Module):
    ...
    """
    def __init__(self, label_num, masks, backbone='resnet34', model_path="./resnet34-333f7ec4.pth"):
    ...
    def bbox_view(self, src, loc, conf, mask):
        ret = []
        for i, (s, l, c) in enumerate(zip(src, loc, conf)):
            #print(i,mask[i],l(s).view(s.size(0), 4, -1).size())
            if mask[i]:
    ...
    def forward(self, data):
        ...
        mask = self.masks[data.size()[3]]
        index = [i for i, e in enumerate(mask) if e != 0][-1]
        ...
        locs, confs = self.bbox_view(src, self.loc, self.conf, mask)
